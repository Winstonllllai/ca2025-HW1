...
bf16_sqrt.loop1:
    ...
    li t6, 0        # t6 = i = 0
    li s1, 16       # s1 = dummy = 16
bf16_sqrt.mul_loop:
    bge t6, s1, bf16_sqrt.mul_end  # if i >= 16 goto mul_end
    andi s0, t5, 1    # s0 = dummy = sq & 1
    beq s0, zero, bf16_sqrt.skip_add1
    add t5, t5, t4   # sq += mid
bf16_sqrt.skip_add1:
    srli t5, t5, 1    # sq >>= 1
    addi t6, t6, 1    # i++
...
bf16_lt.not_zero:
    ...
    bge s2, s1, bf16_lt.less  # if sign_a <= sign_b goto less
    li a0, 1       # return 1
    lw s2, 12(sp)       # Restore s2
    lw s1, 8(sp)        # Restore s1
    lw s0, 4(sp)        # Restore s0
...
bf16_lt.same_sign:
    ...
    bge a1, s0, bf16_lt.less1  # if b >= a goto less
    li a0, 1       # return 1
    lw s2, 12(sp)       # Restore s2
    lw s1, 8(sp)        # Restore s1
    lw s0, 4(sp)        # Restore s0
...
bf16_lt.less1:
    ...
    ret
bf16_lt.positive:
    bge s0, a1, bf16_lt.less  # if a >= b goto less
    li a0, 1       # return 1
    lw s2, 12(sp)       # Restore s2
    lw s1, 8(sp)        # Restore s1
    lw s0, 4(sp)        # Restore s0
